...
int main (void){
  ...
  malloc_init (); /*Dependent on the lock system being up*/
  paging_init ();
  ...
  intr_init ();
  timer_init (); /*Requires interrupts to be enabled in order to initialize the PIC*/
  kbd_init (); /*Registers the keyboard interrupt*/
  input_init ();
#ifdef USERPROG
  exception_init ();
  syscall_init ();
...
static void run_actions (char **argv){
	...
	/* Table of supported actions. ARRAY */
